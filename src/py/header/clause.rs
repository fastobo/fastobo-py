...
impl DateClause {
    #[new]
    fn __init__<'py>(date: &Bound<'py, PyDateTime>) -> PyClassInitializer<Self> {
    ...
    #[setter]
    fn set_date<'py>(&mut self, dt: &Bound<'py, PyDateTime>) {
...
impl Display for SubsetdefClause {
    fn fmt(&self, f: &mut Formatter) -> FmtResult {
        let clause: fastobo::ast::HeaderClause =
...
impl SubsetdefClause {
    ...
    fn get_subset<'py>(
        &self,
        py: Python<'py>,
    ) -> PyResult<<&'py Ident as IntoPyObject<'py>>::Output> {
        (&self.subset).into_pyobject(py)
    }
...
impl SynonymTypedefClause {
    #[new]
    #[pyo3(signature = (typedef, description, scope = None))]
    fn __init__<'py>(
        typedef: Ident,
        description: String,
        scope: Option<&Bound<'py, PyString>>,
...
impl From<IdspaceClause> for obo::HeaderClause {
    fn from(clause: IdspaceClause) -> Self {
        let url = Python::with_gil(|py| clause.url.bind(py).borrow().clone_py(py)).into();
...
impl IdspaceClause {
    ...
    fn raw_value(&self) -> String {
        Python::with_gil(|py| {
            ...
            let url = &*self.url.bind(py).borrow();
